...
/* === Tilted Card Interaction === */
document.querySelectorAll('[data-tilted-card]').forEach(card => {
  const inner = card.querySelector('[data-tilted-inner]');
  const img = card.querySelector('[data-tilted-img]');
  const caption = card.querySelector('[data-tilted-caption]');
  const rotateAmplitude = parseFloat(card.dataset.tiltedRotate) || 14;
  const scale = parseFloat(card.dataset.tiltedScale) || 1.1;
